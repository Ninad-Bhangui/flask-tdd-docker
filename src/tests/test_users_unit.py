...
def test_add_user(test_app, monkeypatch):
    ...
    resp = client.post("/users",data=json.dumps({"username": "michael", "email": "johndoe@seven.com"}),content_type="application/json")
    data = json.loads(resp.data.decode())
    assert resp.status_code == 201
    assert "johndoe@seven.com was added!" in data["message"]
...
def test_remove_user(test_app, monkeypatch):
    ...
    assert "user-to-be-removed was removed!" in data["message"]
